...
<body>
	...
	<div class="divi">
		<h1 align="center">Received Contact</h1>
		<table class="w3-table-all">
			...
			<%
				try {
					...
					PreparedStatement mySmt = myCon.prepareStatement("select * from Contact where Status='Received' or Status='Pending'or Status='Open'or Status='Action Under Process'");
					ResultSet myRes = mySmt.executeQuery();
					if (myRes.next() == false) {
						response.sendRedirect("LoginContactComplete.jsp");
					} else {
						myRes.beforeFirst();
					while (myRes.next()) {
						out.print("<tr>");
			...
			<select style="width: 230px;" name="sno" class="mail" required>
				...
				<%
					Connection myCon1 = DBConnection.getConnection();
						...
						ResultSet myRes1 = mySmt1.executeQuery("select SNo from Contact where Status='Received' or Status='Pending'or Status='Open'or Status='Action Under Process'");
						while (myRes1.next()) {
